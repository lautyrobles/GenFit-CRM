...
import { Search, User, Mail, Phone, CreditCard, Calendar, Edit3, X, ArrowLeft } from 'lucide-react'
...
const Clientes = () => {
  ...
  useEffect(() => {
    ...
    const fetchPagos = async () => {
      ...
      try {
        ...
        const pagosCliente = data.filter(p => (p.users?.id === cliente.id) || (p.user_id === cliente.id))
  ...
  const handleBuscar = async () => {
    ...
    try {
      ...
      if (resultados.length > 1) {
        setClientesCoincidentes(resultados); setMostrarModal(true);
  ...
  const handleGuardarEdicion = async (e) => {
    ...
    try {
      ...
      setCliente(actualizado); setMostrarModalEdicion(false);
    ...
    <section className={styles.clientesLayout}>
      <div className={styles.searchSection}>
        <div className={styles.searchHeader}>
          ...
          {cliente && (
            <button className={styles.btnBack} onClick={() => {setCliente(null); setBusquedaRealizada(false);}}>
          ...
          {cliente && (
            <div className={styles.dashboardGrid}>
              ...
              <div className={`${styles.card} ${styles.asistenciaCard}`}>
                <div className={styles.cardHeader}><h3>Control Asistencia</h3><Calendar size={18}/></div>
          ...
          <div className={styles.modalContent}>
            ...
            <form className={styles.modalBody} onSubmit={handleGuardarEdicion}>
              ...
              <div className={styles.modalActions}>
                ...
                <button type="submit" className={styles.btnSave} disabled={saving}>{saving ? "Guardando..." : "Guardar"}</button>
              </div>
            </form>
          </div>
        </div>
